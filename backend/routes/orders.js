...
// @desc    Buyer requests order cancellation (only Pending or Processing)
// @access  Private (Buyer - own order only)
  ...
  try {
    ...
    if (order.status === 'Shipped' || order.status === 'Delivered') {
      return res.status(400).json({
        success: false,
        message: 'Cannot cancel order that has been shipped or delivered. Use the Return feature for delivered items.'
      });
    }

    if (order.status === 'Cancellation Requested' || order.status === 'Cancelled') {
      return res.status(400).json({
        success: false,
    ...
    const buyerName = order.userId?.username || order.userId?.email || 'A buyer';
    ...
    const notificationPromises = admins.map((admin) =>
      Notification.create({
        ...
        message: `${buyerName} has requested to cancel Order #${orderIdShort}.`,
        relatedId: order._id,
        isRead: false
      })
    );
    await Promise.allSettled(notificationPromises).catch((err) => console.error('Cancel notification error:', err));
...
// @desc    Admin approve or reject cancellation request
...
router.put('/:id/handle-cancellation', adminMiddleware, [
  ...
  try {
    ...
    if (order.status !== 'Cancellation Requested') {
      return res.status(400).json({
        success: false,
        message: `No cancellation request to handle. Order status: ${order.status}`
      });
    }
    ...
    if (action === 'Approve') {
      ...
      order.cancellationRequest.adminResponse = adminResponse || 'Approved';
      await order.save();

      const orderItems = await OrderItem.find({ orderId: order._id }).populate('productId', 'stockQuantity');
      ...
      await Notification.create({
        ...
        message: `Your cancellation request for Order #${orderIdShort} has been approved.`,
        relatedId: order._id,
        isRead: false
      });
    } else {
      order.status = 'Pending';
      order.cancellationRequest = { isRequested: false, reason: null, requestDate: null, adminResponse: adminResponse || null };
      await order.save();

      const rejectMsg = adminResponse
        ? `Your cancellation request for Order #${orderIdShort} was rejected. Reason: ${adminResponse}`
        : `Your cancellation request for Order #${orderIdShort} was rejected.`;
      await Notification.create({
        recipientId: order.userId._id,
        senderId: req.user._id,
        type: 'ORDER_CANCELLATION_UPDATE',
    ...
    res.status(500).json({ success: false, message: error.message || 'Failed to process cancellation' });
  }
});
...
router.put('/:id/status', authorize('Seller', 'Admin'), [
  body('status').isIn(['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled', 'Cancellation Requested'])
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
